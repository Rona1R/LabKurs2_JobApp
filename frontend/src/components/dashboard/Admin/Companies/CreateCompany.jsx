...
export default function CreateCompany(props) {
  ...
  const validateName = async () => {
    try {
      const nameValid = await companyService.validate(formData.name);
      if (!nameValid.data) {
  ...
  const uploadImage = async (fileData) => {
    try {
      ...
      if (err.response && err.response.status === 400) {
        ...
        showNotification("error","An Unexptected Error Occurred while uploading image!")
        props.handleClose();
  ...
  const createCompany = async (data) => {
    try {
      ...
      props.refresh();
      showNotification("success","Company was successfully created!")
      props.handleClose();
    } catch (err) {
      showNotification("error","An Unexptected Error Occurred!")
      props.handleClose();
            ...
            >
              <Grid container spacing={2}>
                ...
                <Grid size={12} sx={{ marginBottom: 3 }}>
                  <Form.Label className="logo-label">Upload Logo *</Form.Label>
                  <Form.Control
                    type="file"
                    accept="image/*"
