...
export default function UpdateExperience(props) {
    ...
    const fetchData = async () => {
      try {
        ...
        if(response.data.companyId){
            const compResponse = await companyService.getById(
              response.data.companyId
            );
            setSelectedCompany(compResponse.data);
        }
      } catch (err) {
        console.log(err);
        handleClose();
  ...
  const handleSubmit = async () => {
    ...
    //   console.log(formData);
      ...
      //   console.log(data);
      try {
        await experienceService.update(props.id,data);
        props.refresh();
        showNotification("success","Experience was successfully updated!");
      } catch (err) {
