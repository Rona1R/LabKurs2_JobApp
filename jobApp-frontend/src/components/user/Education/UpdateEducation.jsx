...
export default function UpdateEducation(props) {
  ...
  const { showNotification } = useNotification();
   ...
   useEffect(() => {
      const fetchData = async () => {
        try {
          const response = await educationService.getById(props.id);
          const data = {
              ...response.data,
              startDate: dayjs(response.data.startDate),
              endDate: response.data.endDate === '0001-01-01' || response.data.endDate === null? null : dayjs(response.data.endDate)
          }
          setFormData(data);
        //   console.log(data);
          if(response.data.institutionId){
              const insResponse = await institutionService.getById(
                response.data.institutionId
              );
              setSelectedInstitution(insResponse.data);
          }
        } catch (err) {
          console.log(err);
          handleClose();
  ...
  const validate = () => {
    if (formData.manualInstitutionName.trim() === "" && formData.institutionId === null) {
      setInstitutionError(
  ...
  const handleSubmit = async () => {
   ...
   //   console.log(formData);
    ...
    //   console.log(data);
        try {
          await educationService.update(props.id,data);
          props.refresh();
          showNotification("success","Education was successfully updated!");
        } catch (err) {
          console.log(err);
