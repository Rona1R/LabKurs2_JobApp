...
export default function UpdateLanguage(props) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await userLanguageService.getById(props.id);
        setFormData(response.data);

        if(response.data.languageId){
            const languageRes = await languageService.getById(response.data.languageId);
            setSelectedLanguage(languageRes.data);
        }
      } catch (err) {
        console.log(err);
        handleClose();
  ...
  const handleSubmit = async () => {
    ...
    //   console.log(formData);
        try {
          await userLanguageService.update(props.id,formData);
          props.refresh();
